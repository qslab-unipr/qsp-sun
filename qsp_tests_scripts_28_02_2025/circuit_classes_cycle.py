...
class QuantumCircuit():
    ...
    def computeQuantumState(self,  modulo : bool = False, state_vector : np.ndarray = np.array([])):
        if state_vector.size == 0:
            out_state = self.circuitState(self._input_register, self._wires)()
        else:
            out_state = self.circuitState(state_vector, self._wires)()
        
        if modulo == True:
            return np.array([np.sqrt(np.real(x)**2 + np.imag(x)**2) for x in out_state])
        
        return out_state
    
    def getCircuitInfos(self):
        qnode = self.circuitState(self._input_register, self._wires)
        specs = qml.specs(qnode)()
        return specs
    
    def printMetrics(self, vec1 : np.ndarray, vec2 : np.ndarray):
            print(f"\nFidelity: {qml.math.fidelity_statevector(vec1, vec2)}")
            print(f"\nTrace distance: {qml.math.trace_distance(qml.math.reduce_statevector(vec1, [0]), qml.math.reduce_statevector(vec2, [0]))}")            
